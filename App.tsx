...
export default function App() {
  ...
  const handleSaveAsTemplate = () => {
    const newSavedTemplate: SavedTemplate = {
      id: `saved_${Date.now()}`,
      tag: resumeData.currentTag || 'Untitled',
      baseTemplate: selectedTemplate,
      data: { ...resumeData } // Create a deep copy
    };
    setSavedTemplates(prev => [newSavedTemplate, ...prev]);
    // Optionally, navigate to My Templates page after saving
  ...
  const renderContent = () => {
    switch (currentView) {
      ...
      case View.MY_TEMPLATES:
        return <MyTemplates templates={savedTemplates} onLoadTemplate={handleLoadSavedTemplate} />;
      case View.EDITOR:
        return (
          <Editor 
            data={resumeData} 
  ...
  if (currentView === View.LANDING) {
     return <div className="min-h-screen w-full bg-brand-bg">{renderContent()}</div>;
  }

  if (currentView === View.EDITOR) {
    return <div className="h-screen w-screen overflow-hidden bg-brand-bg font-sans text-brand-dark">{renderContent()}</div>;
  }
        ...
        <div className="p-4 border-t border-brand-border bg-brand-secondary/30">
          <div className={`flex items-center gap-3 ${isSidebarCollapsed ? 'flex-col space-y-3' : 'justify-between'}`}>
            <div className={`flex items-center gap-3 w-full ${isSidebarCollapsed ? 'justify-center' : ''}`}>
              <div className="w-10 h-10 rounded-full bg-brand-dark text-brand-green flex items-center justify-center font-bold text-sm shadow-sm shrink-0">JO</div>
